	    ...
	    <div id="dhs-billing-form">
		    ...
		    <div style="padding:10px;">
		      ...
		      <div class="input-append" style="margin-bottom:0px;">
		      	<input type="text" class="medicines-select input-small" id="medicine_name0" name="medicine[0][medicine_name]" placeholder="Product Name" list="productNameList" />
...
<br /><br />
		...
		<div class="row-fluid" id="calculate_total">
			<div class="span5 offset3 well">
	            ...
	            <div class="row-fluid" style="padding-bottom:10px;">
		            ...
		            <div class="span8">
		            	<input type="text" id="grand_total" name="grand_total" class="input-small" readonly="readonly">
		            	<button class="btn btn-success" id="cal_total_btn" type="button">Calculate</button>
		            </div>
	            </div>
	    	</div>
	    	<div class="span4">&nbsp;</div>      
    ...
    <datalist id="productNameList">
        <?php  $getProductName="SELECT DISTINCT MD.medicine_name FROM medicine AS MD   
                                INNER JOIN new_inventory AS NI ON NI.medicine_id=MD.medicine_id
                                WHERE MD.minLimit < NI.quantity " ;
               
               $Productresult=mysqli_query($con,$getProductName);
               while($row=mysqli_fetch_row($Productresult))
               {
		...
		<div class="row-fluid" id="customer_details">
			<div class="well span5 offset3">
				<div class="row-fluid" style="padding-bottom:20px;">
	            	<div class="span5"><label class="control-label" for="grand_total">Grand Total:</label></div>
		            <div class="span7">
		            	<input type="text" id="n_grand_total" name="grand_total" class="input-small" readonly="readonly">
		            	<button class="btn btn-success" id="recal_total_btn" type="button">Re Calculate</button>
		            </div>
	            </div>
	            
				<div class="row-fluid" style="padding-bottom:20px;">
	            	<div class="span5"><label class="control-label">Reference :</label></div>
		            <div class="span7 prescribed-by">
		            	<div class="btn-group" data-toggle="buttons-radio">
						  <button type="button" class="btn" id="prescribed-yes">Yes</button>
						  <button type="button" class="btn active" id="prescribed-no">No</button>
						</div>
		            </div>
	            </div>
	     ...
	     <!--       <div id="doctor_details">
	            	<div class="row-fluid" style="padding-bottom:20px;">
		            	<div class="span5"><label class="control-label" for="doctor_name">Reference Name:</label></div>
			            <div class="span7">
				         			<?php
                                         //$doctors = array(); 
                                         //$result = mysqli_query($con,"select `doctor_id`, `doctor_name`, `mobile` from `doctors` where `store_key` = '$store_key'");
                                         //while ($doc = mysqli_fetch_array($result)):
                                         //    $doctors[$doc['doctor_name']] = $doc['mobile'];
                                         //endwhile;?>
				         			
				         		<input id="doctor_name" autocomplete='off' class="typeahead" type="text" value="" name="doctor_name" />
				         		
				         		<script type="text/javascript">
				         			(function($){
				         				var data = {};
				         				data.source = <?php// echo "['". implode("','", array_keys($doctors)) ."']";?>;
				         				$("#doctor_name").typeahead(data);
					         		})(jQuery);
				         		</script>
			                </div>
		              </div>
		            ...
		            <div class="row-fluid" style="padding-bottom:20px;">
		            	<div class="span5"><label class="control-label" for="doctor_no">Reference  Mobile:</label></div>
			            <div class="span7"><input type="text" id="doctor_no" name="doctor_no"></div>
		            </div>
	            </div>-->
	            
	            <div class="row-fluid" style="padding-bottom:20px;">
	            	<div class="span5"><label for="customer_name">Customer Name:</label></div>
	                <div class="span7"><input type="text" id="customer_name" name="customer_name" required=""></div>
	            </div>
	            
	              
	            <div class="row-fluid" style="padding-bottom:20px;">
	            	<div class="span5"><label class="control-label" for="customer_no">Customer Mobile:</label></div>
		            <div class="span7"><input type="text" id="customer_no" name="customer_no" required=""></div>
	            </div>
	            
	            <div class="row-fluid" style="padding-bottom:20px;">
					<div class="span5"><label for="payment_mode">Payment Mode:</label></div>
		            <div class="span7">
		            	<select name="payment_mode" class="payment_mode">
	            ...
	            <div class="row-fluid" style="padding-bottom:20px;" id="cheque">
					<div class="span5"><label for="cheque_no">Cheque No.:</label></div>
		            <div class="span7">
		            	<input type="text" id="cheque_no" name="cheque_no" required="" value="0">
		             </div>
	            </div>
	            
	            <div class="row-fluid" style="padding-bottom:20px;" id="bank_name">
					<div class="span5"><label for="cheque_no">Bank Name:</label></div>
		            <div class="span6">
		            	<input type="text" name="bank_name" required="" value="0">
		             </div>
	            </div>
	            
	            <div class="row-fluid" style="padding-bottom:20px;" id="branch_name">
					<div class="span5"><label for="branch_name">Branch Name:</label></div>
		            <div class="span6">
		            	<input type="text" name="branch_name" required="" value="0">
		             </div>
	            </div>
	            ...
	            <div class="row-fluid">
	            	<div class="span5">&nbsp;</div>
		            <div class="span6">
						<input type="submit" name="btn_save" class="btn btn-primary" value="Print" id="btn_save" />
					</div>
	            </div>
			</div>
...
(function($){
	$(document).ready(function(){
		...
		$("#recal_total_btn").click(function(){
			$("#customer_details").hide();
			$("#calculate_total").show();
		});
		...
		$(".bill-data").livequery('change', function () {
            ...
            //chk quantity 
			var inStock = $('#inStock' + row_id).is(':checked');
			var availQuantity = parseInt($('#remaining' + row_id).val());

			
			if (inStock == 'true' && availQuantity < quantity) {
		...
		$("#prescribed-yes").click(function(){
			$("#doctor_details").show();
		});

		$("#prescribed-no").click(function(){
			$("#doctor_name").val('');
			$("#doctor_no").val('');
			$("#doctor_details").hide();
		});
...
//$("#doctor_name").change(function(){
	//		var doctor = $(this).val();
	    ...
	    //var obj = JSON.parse(doctors);
	    //var mobile = obj[doctor];
	    //$("#doctor_no").val(mobile);
	    //});
	    ...
	    $(".data-barcode").livequery('focusout', function () {
	        ...
	        var batchCode = $('#' + barcode_id).val();

	        $.ajax({
	            type: 'POST',
	            //url: 'index.php?view=manage_medicine',
	            url: 'Admin/db/getMedicineDetails.php',
	            data: {
	                data: 'data',
	                action: 'batch_code',
	                batchCode: batchCode
	            ...
	            success: function (details) {
	                ...
	                if (obj.status == 'Success') {
	                    var medicine_id = obj.data.medicine_id;
	                    var med_name = obj.data.medicine_name;
	                    var remaining = obj.data.quantity;
	                    var unitCost = obj.data.unit_cost;
	                    // alert(unitCost);
	                    $('#name' + row).val(medicine_id);
	                    $('#medicine_name' + row).val(med_name);
	                    $('#remaining' + row).val(remaining);
	                    $('#cost' + row).val(parseFloat(unitCost).toFixed(2));
	                    $('#quantity' + row).focus();
	    ...
	    $(".medicines-select").livequery('focusout', function () {
	        //$(".medicines-select").blur(function () {
	        var med_id = $(this).attr('id');;
	        var row = med_id.substr(13, 1);
	        var med_name = $('#' + med_id).val();
	        ...
	        $.ajax({
	            ...
	            success: function (details) {
	                ...
	                if (obj.status == 'Success') {
	                    var medicine_id = obj.data.medicine_id;
	                    var batch_code = obj.data.batch_code;
	                    var remaining = obj.data.quantity;
	                    var unitCost = obj.data.unit_cost;

	                    // alert(unitCost);
	                    $('#name' + row).val(medicine_id);
	                    $('#barcode' + row).val(batch_code);
	                    $('#remaining' + row).val(remaining);
	                    $('#cost' + row).val(parseFloat(unitCost).toFixed(2));
	                    $('#quantity' + row).focus();
    ...
    function getNewRow(count) {
        ...
        html += ' <input type="checkbox" name="medicine[' + count + '][inStock]"   id="inStock' + count + '" class="input-small" placeholder="Barcode" />InStock';
        html += '<input type="text" id="barcode' + count + '" name="medicine[' + count + '][barcode]" placeholder="batchCode" class="input-small data-barcode numeric-txt" style="margin-left:5px;">';
        ...
        html += '<input type="text" class="medicines-select input-small" id="medicine_name' + count + '" name="medicine[' + count + '][medicine_name]" placeholder="Medicine Name" list="productNameList" />';

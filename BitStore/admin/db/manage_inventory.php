...
if(isset($_POST['query'])){
    ...
    $bill_no_count="SELECT `Amount`,`BillDetails`,PendiangAmount FROM `vendor_payment` where `BillNo`='$bill_no' AND `ven_id`='$dist_id' AND `date`='$now'";
    ...
    if($bill_result  && $bill_result->num_rows > 0){
        ...
        $pendingAmount=$old_data[2]+($quantity * $buy_cost );
        $update_query=" UPDATE `vendor_payment`  SET `Amount`='$newAmount',`BillDetails`='$billDetails',`PendiangAmount`='".$pendingAmount."' WHERE `BillNo`='$bill_no' AND `ven_id`='$dist_id' AND `date`='$now'"  ;
    ...
    else
    {
        ...
        $insert_vendor_payment="INSERT INTO `vendor_payment`(`ven_id`,`BillNo`,`BillDetails`, `Amount`, `PendiangAmount`, `paymentDate`, `date`)
             VALUES ('$dist_id','$bill_no','$billDetail','$amount',$amount,'$payment_date','$now')";
